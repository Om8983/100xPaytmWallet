...
// export const AmountCardContainer = React.memo(
//     ({ children, id, className, cardRefs, isWrapperHovered, glowPositions, glowAngles, cardActive }: Props) => {
...
const SPREAD = 40; // arc width in degrees
const BORDER_WIDTH = "1px";
...
// ─── AmountCardContainer ──────────────────────────────────────────────────────
// Wrapped in React.memo so it never re-renders from parent mouse-move state.
// Glow is driven purely via CSS custom properties written directly to the DOM.
        ...
        ({ children, id, className, cardRefs, absoluteContainerClassname }: AmountCardContainerProps) => {
            ...
            return (
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8, ease: "easeInOut" }}
                    // CSS vars written by parent's mousemove handler land here:
                    // --glow-angle  (number, degrees)
                    // --glow-active (0 or 1)
                    style={
                        {
                            "--glow-angle": "0",
                ...
                >
                    ...
                    <div
                        ...
                        className="pointer-events-none absolute inset-0 rounded-[inherit]"
                        style={{
                            // opacity driven by CSS var — no JS needed
                    ...
                    >
                        <div
                            ...
                            style={{
                                ...
                                conic-gradient(
                                from calc((var(--glow-angle) - ${SPREAD}) * 1deg),
                                #00000000 0deg,
                                #fff,
                                #00000000 calc(${SPREAD} * 2deg)
                                )
